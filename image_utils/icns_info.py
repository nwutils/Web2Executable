...
type_dict = {
    "mask": {
        128: {8: ICNS_128x128_8BIT_MASK},
        48: {1: ICNS_48x48_1BIT_MASK, 8: ICNS_48x48_8BIT_MASK},
        32: {1: ICNS_32x32_1BIT_MASK, 8: ICNS_32x32_8BIT_MASK},
        16: {1: ICNS_16x16_1BIT_MASK, 8: ICNS_16x16_8BIT_MASK},
    },
    "data": {
        1024: {32: ICNS_1024x1024_32BIT_ARGB_DATA},
        512: {32: ICNS_512x512_32BIT_ARGB_DATA},
        256: {32: ICNS_256x256_32BIT_ARGB_DATA},
        128: {32: ICNS_128x128_32BIT_DATA},
        48: {
            1: ICNS_48x48_1BIT_DATA,
            4: ICNS_48x48_4BIT_DATA,
            8: ICNS_48x48_8BIT_DATA,
            32: ICNS_48x48_32BIT_DATA,
        },
        32: {
            1: ICNS_32x32_1BIT_DATA,
            4: ICNS_32x32_4BIT_DATA,
            8: ICNS_32x32_8BIT_DATA,
            32: ICNS_32x32_32BIT_DATA,
        },
        16: {
            1: ICNS_16x16_1BIT_DATA,
    ...
    for colorOffset in range(3):
        ...
        for dataInCount in range(1, dataInChanSize):
            ...
            if runLength < 2:
                ...
                if (
                    dataByte == dataRun[runLength - 1]
                    and dataByte == dataRun[runLength - 2]
                ):
                    runType = 1
                else:
                    runType = 0
                ...
                if runType == 0 and runLength < 128:
                    ...
                    if runType == 0:
                        ...
                        dataTemp[dataTempCount : dataTempCount + runLength] = dataRun[
                            :runLength
                        ]
    ...
    while color_offset < 3:
        ...
        while (pixel_offset < pixel_count) and (data_offset < len(data)):
            if (data[data_offset] & 0x80) == 0:
                ...
                while (
                    (i < run_length)
                    and (pixel_offset < pixel_count)
                    and (data_offset < len(data))
                ):
                    dest_icon_data[(pixel_offset * 4) + color_offset] = data[
                        data_offset
                    ]
                    pixel_offset += 1
                    data_offset += 1
...
def get_mask_type_for_icon_type(icon_type):
    if (
        icon_type == ICNS_TABLE_OF_CONTENTS
        or icon_type == ICNS_ICON_VERSION
        or icon_type == ICNS_1024x1024_32BIT_ARGB_DATA
        or icon_type == ICNS_512x512_32BIT_ARGB_DATA
        or icon_type == ICNS_256x256_32BIT_ARGB_DATA
    ):
    ...
    def dump(self):
        ...
        for field in self._fields:
            ...
            if not isinstance(val, (str, bytes)):
                ...
                values.extend(
                    struct.unpack(
                        ">" + "B" * len(val), bytearray(val, encoding="utf-8")
                    )
                )
...
class ICNSInfo(Printable):
    ...
    def get_image_type(self):
        ...
        if self.iconBitDepth == 0 and (
            self.iconSize.width < 128 or self.iconSize.height < 128
        ):
    ...
    def from_type(cls, type):
        ...
        icon_info.iconRawDataSize = int(
            icon_info.iconSize.height
            * icon_info.iconSize.width
            * icon_info.iconBitDepth
            / ICNS_BYTE_BITS
        )
...
class ICNSHeader(Structure):
    _fields = [Field("TypeID", 4, "icns"), Field("Size", 4)]
    ...
    def parse_image(self, image):
        ...
        for icon_s in icon_sizes:
            ...
            if mask_type != ICNS_NULL_MASK:
                ...
                while (
                    iconDataOffset < icns_info.iconRawDataSize
                    and maskDataOffset < icns_mask.iconRawDataSize
                ):
...
class ICNSElement(Structure):
    _fields = [Field("TypeID", 4), Field("Size", 4)]
    ...
    def get_image(self):
        ...
        if icon_type in [
            ...
            magic_png = bytearray([0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A])
            magic_read = data[:8]
            ...
            if icon_type in [
                ...
                icon_data_row_size = (
                    icns_info.iconSize.width * icon_bit_depth * ICNS_BYTE_BITS
                )
                ...
                if raw_data_size < icns_info.iconRawDataSize:
                    ...
                    while data_count < icns_info.iconSize.width:
                        ...
                        icns_info.data[data_pos : data_pos + icon_data_row_size] = data[
                            data_pos : data_pos + icon_data_row_size
                        ]
                        data_count += 1
                    pixel_count = icns_info.iconSize.width * icns_info.iconSize.height
                    data_count = 0
    ...
    def get_mask(self):
        ...
        mask_data_row_size = int(
            icns_info.iconSize.width * mask_bit_depth / ICNS_BYTE_BITS
        )
        ...
        if mask_type in [
            ...
            while data_count < icns_info.iconSize.height:
                ...
                icns_info.data[data_pos : data_pos + mask_data_row_size] = data[
                    data_pos : data_pos + mask_data_row_size
                ]
                data_count += 1
...
def icns_read_be(icns_data, size):
    ...
    if size == 1:
        ...
        return (
            icns_bytes[2] & 0xFFFF
            | icns_bytes[1] & 0xFFFF << 8
            | icns_bytes[0] & 0xFFFF << 16
        ) & 0x00FFFFFF
    elif size == 4:
        return (
            icns_bytes[3]
...
def get_image_with_mask(icns_data, element_type):
    ...
    im = Image.frombytes(
        "RGBA",
        [icns_image.iconSize.width, icns_image.iconSize.height],
        bytes(icns_image.data),
    )
